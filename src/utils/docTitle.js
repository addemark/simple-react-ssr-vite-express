...
export const getTitleFromRoute = (path) => {
  if (titleMap[path]) {
    return `${titleMap[path]} | BrandBell Reviews`;
  }
  ...
  if (userProfileRegex.test(path)) {
    return "User Profile |  BrandBell Reviews";
  } else if (postRegex.test(path)) {
    return "Post |  BrandBell Reviews";
  } else if (communityRegex.test(path)) {
    return "Community |  BrandBell Reviews";
  }

  return " BrandBell Reviews";
};
